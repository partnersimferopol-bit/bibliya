...
const categories: { id: QuestionMode; label: string }[] = [
  ...
  { id: "general", label: `${CATEGORY_ICONS.general} ${CATEGORY_LABELS.general}` },
];
  ...
  return (
    <div className="max-w-lg mx-auto p-4 sm:p-6 scroll-border rounded-xl w-full">
      ...
      <p className="text-parchment/70 text-sm text-center mb-4">
        Один игрок: верный ответ — выстрел; неверный — следующий вопрос
      </p>
      ...
      <div className="flex gap-3">
        ...
        <Button
          className="flex-1"
          onClick={() => startGame("ai", qMode, aiDiff, name || "Капитан")}
        >
          Начать расстановку →
        </Button>
      </div>
